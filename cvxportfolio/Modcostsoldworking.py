...
#				+ dm.time_locator(self.Lambda, t).tolist()[0]-dm.time_locator(self.Lambda, t+dt.timedelta(hours=1)).tolist()[0]    #Late requests and failure cost
			...
			third_term = dm.time_locator(self.failure, t+dt.timedelta(hours=2)).multiply(dm.time_locator(self.Lambda, t+dt.timedelta(hours=2)).multiply(self.L).tolist()[0])\
				+dm.time_locator(self.Lambda, t+dt.timedelta(hours=LA+1)).tolist()[0]- dm.time_locator(self.Lambda, t+dt.timedelta(hours=LA)).tolist()[0] #dm.time_locator(self.Lambda, t).tolist()[0]    #Late requests and failure cost
	                second_term = dm.time_locator(self.pricePerReq,t+dt.timedelta(hours=2)).multiply(dm.time_locator(self.Lambda, t+dt.timedelta(hours=2)).tolist()[0])    #Provisioning cost
			#	+dm.time_locator(self.Lambda, t).tolist()[0]- dm.time_locator(self.Lambda, t).tolist()[0]    #Late requests and failure cost
#				+ dm.time_locator(self.Lambda, t).tolist()[0]-dm.time_locator(self.Lambda, t+dt.timedelta(hours=1)).tolist()[0]    #Late requests and failure cost
...
#                second_term = dm.time_locator(self.pricePerReq,t+dt.timedelta(hours=1)).multiply(dm.time_locator(self.Lambda, t).tolist()[0])    #Provisioning cost
	...
	u[u<0]=0
        u_nc = u
        if LA==1: #dm.time_locator(self.Lambda, t+dt.timedelta(hours=1)).tolist()[0]- dm.time_locator(self.Lambda, t).tolist()[0]>0:
	       	if dm.time_locator(self.Lambda, t+dt.timedelta(hours=2)).tolist()[0]- dm.time_locator(self.Lambda, t).tolist()[0]>0:
			third_term = dm.time_locator(self.failure, t+dt.timedelta(hours=2)).multiply(dm.time_locator(self.Lambda, t+dt.timedelta(hours=2)).multiply(self.L).tolist()[0])\
				+dm.time_locator(self.Lambda, t+dt.timedelta(hours=2)).tolist()[0]- dm.time_locator(self.Lambda, t).tolist()[0]    #Late requests and failure cost
		        second_term = dm.time_locator(self.pricePerReq,t+dt.timedelta(hours=2)).multiply(dm.time_locator(self.Lambda, t+dt.timedelta(hours=2)).tolist()[0])    #Provisioning cost
			print "Debugging Calcu",LA,t, third_term*u, second_term*u, sum(third_term*u),sum(second_term*u), dm.time_locator(self.Lambda, t+dt.timedelta(hours=2)).tolist()[0],dm.time_locator(self.Lambda, t+dt.timedelta(hours=LA)).tolist()[0]
#			print "Debugging Calcu",LA,t, third_term*u, second_term*u, sum(third_term*u),sum(second_term*u), dm.time_locator(self.Lambda, t+dt.timedelta(hours=2)).tolist()[0],dm.time_locator(self.Lambda, t+dt.timedelta(hours=LA)).tolist()[0]
#			print "Debugging Calcu",LA,t, third_term*u, second_term*u, sum(third_term*u),sum(second_term*u)
			print "error in prediction",dm.time_locator(self.Lambda, t+dt.timedelta(hours=2)).tolist()[0]- dm.time_locator(self.Lambda, t).tolist()[0],"LLLL", self.L
		else:
			third_term = dm.time_locator(self.failure, t+dt.timedelta(hours=2)).multiply(dm.time_locator(self.Lambda, t+dt.timedelta(hours=2)).multiply(self.L).tolist()[0])
	                second_term = dm.time_locator(self.pricePerReq,t+dt.timedelta(hours=2)).multiply(dm.time_locator(self.Lambda, t+dt.timedelta(hours=2)).tolist()[0])    #Provisioning cost
			print "Debugging Calcu",LA,t, third_term*u, second_term*u, sum(third_term*u),sum(second_term*u), dm.time_locator(self.Lambda, t+dt.timedelta(hours=2)).tolist()[0],dm.time_locator(self.Lambda, t+dt.timedelta(hours=LA)).tolist()[0]
...
#			print "Debugging Calcu", LA,t, third_term*u, second_term*u, sum(third_term*u),sum(second_term*u),"LLLLL",self.L
	       	...
	       	if dm.time_locator(self.Lambda, t+dt.timedelta(hours=2)).tolist()[0]- dm.time_locator(self.Lambda, t+dt.timedelta(hours=LA)).tolist()[0]>0:
			third_term = dm.time_locator(self.failure, t+dt.timedelta(hours=2)).multiply(dm.time_locator(self.Lambda, t+dt.timedelta(hours=2)).multiply(self.L).tolist()[0])\
				+dm.time_locator(self.Lambda, t+dt.timedelta(hours=2)).tolist()[0]- dm.time_locator(self.Lambda, t+dt.timedelta(hours=LA)).tolist()[0]    #Late requests and failure cost
		        second_term = dm.time_locator(self.pricePerReq,t+dt.timedelta(hours=2)).multiply(dm.time_locator(self.Lambda, t+dt.timedelta(hours=2)).tolist()[0])    #Provisioning cost
			print "Debugging Calcu",LA,t, third_term*u, second_term*u, sum(third_term*u),sum(second_term*u), dm.time_locator(self.Lambda, t+dt.timedelta(hours=2)).tolist()[0],dm.time_locator(self.Lambda, t+dt.timedelta(hours=LA)).tolist()[0]
			print "error in prediction",dm.time_locator(self.Lambda, t+dt.timedelta(hours=2)).tolist()[0]- dm.time_locator(self.Lambda, t).tolist()[0],"LLLL", self.L
		else:
			...
			print "Debugging Calcu",LA,t, third_term*u, second_term*u, sum(third_term*u),sum(second_term*u), dm.time_locator(self.Lambda, t+dt.timedelta(hours=2)).tolist()[0],dm.time_locator(self.Lambda, t+dt.timedelta(hours=LA)).tolist()[0]
#			third_term = dm.time_locator(self.failure, t+dt.timedelta(hours=2)).multiply(dm.time_locator(self.Lambda, t+dt.timedelta(hours=2)).multiply(self.L).tolist()[0])\
#				+(dm.time_locator(self.Lambda, t+dt.timedelta(hours=2)).tolist()[0]-dm.time_locator(self.Lambda, t).tolist()[0])*acc
#	                second_term = dm.time_locator(self.pricePerReq,t+dt.timedelta(hours=2)).multiply(dm.time_locator(self.Lambda, t+dt.timedelta(hours=2)).tolist()[0])    #Provisioning cost
